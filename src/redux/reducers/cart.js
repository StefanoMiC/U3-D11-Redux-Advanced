...
const cartReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case REMOVE_FROM_CART:
      return {
        ...state,
        //   content: state.content.filter((book, index) => index !== action.payload),
        content: [
          ...state.content.slice(0, action.payload), // before indexToRemove
          ...state.content.slice(action.payload + 1), // after indexToRemove
        ],
      };
